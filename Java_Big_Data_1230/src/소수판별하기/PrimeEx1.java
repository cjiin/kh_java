...
// 정수 입력 : 12
// 결과 : 2 + 3 + 5 +7 + 11 => 28
// 문제 푸는 방법 : 메소드를 호출 시 정수 값을 입력해서 해당 정수가 소수이면 그 값을 반환
...
public class PrimeEx1 {
    ...
    // 정수 값을 입력받아 해당 정수가 소수인지 아닌지 판별 후 결과 반환하는 메소드
    static int primeSum(int n) {
        // 1과 입력 받은 값을 제외, 즉 2 ~ 입력받은 값 미만
        boolean isPrime = true; // 기본값을 소수로 정함
        for (int i = 2; i < n; i++) {
            if (n % i == 0) isPrime = false; //전달 받은 수를 1과 자기자신을 제외하고 나누어지는 경우를 의미
        }
       if(isPrime) return n;
       else  return 0; //소수가 아닌 애들은 0으로 반환
    }
}
