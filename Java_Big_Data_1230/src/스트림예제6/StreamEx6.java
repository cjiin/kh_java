package 스트림예제6;
// 최종연산 : 중개 연산을 통해 변환된 스트림은 마지막으로 최종 연산을 통해 요소를 소모해 결과를 출력
...
public class StreamEx6 {
    public static void main(String[] args) {
        ...
        //reduce() : 첫번째 요소와 두번째 요소를 가지고 연산을 수행 후, 그 결과와 세번째 요소를 가지고 다시 연산
        ...
        OptionalInt rst2 = stream4.sorted().findFirst(); //정렬 후 첫번째 값
        System.out.println(rst2.getAsInt());
        OptionalInt rst3 = stream5.sorted().findAny(); //병렬처리 관련 내용이 포함되어 있어 요소의 첫번째 값이 아닌 경우가 있음
        System.out.println(rst3.getAsInt());

        // 요소의 검사 :
        // anyMatch() : 해당 스트림의 일부 요소가 조건에 만족하면 true
        // allMatch() : 해당 스트림의 모든 요소가 조건에 만족하면 true
        // noneMatch() : 해당 스트림의 모든 요소가 조건에 만족하지 않을 경우 true
        ...
        System.out.println(stream9.count()); // 4
        System.out.println(stream10.max().getAsInt()); //90
