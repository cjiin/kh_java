...
// 색 : black, brown, red, orange, yellow, green, blue, violet, gery, white
...
public class ResistanceEx {
    public static void main(String[] args) {
        ...
        String[] color = { "black", "brown", "red", "orange", "yellow", "green", "blue", "violet",
                "gery", "white"};
        int fstIndex = 0, secIndex=0, trdIndex=0;
        ...
        for(int i = 0; i < color.length; i++) {
            if (fstColor.equals(color[i])) fstIndex = i; // 첫번째 입력 받은 컬러 문자열과 배열의 컬러이름이 같으면 인덱스를 대입
            if (secColor.equals(color[i])) secIndex = i;
            if (trdColor.equals(color[i])) trdIndex = i;
        }
        //Math.pow(10,지수) 제곱함수를 사용해서 풀기
        //정수값 범위를 벗어날 수 있으므로 long으로 형변환 해줌
        System.out.println((long)((fstIndex * 10) + secIndex) * (long)(Math.pow(10,trdIndex)));
